	...
	useEffect(() => {
		...
		const link = svg
			...
			.on("mouseover", function (event, d) {
				d3.select(this)
					.attr("stroke", "orange") // Change color on hover
				...
				svg
					...
					.attr("y", (d.source.y1 + d.target.y0) / 2)
					.attr("text-anchor", "middle")
					.attr("dy", "-0.5em")
					.attr("font-size", "12px")
					.attr("fill", "#333")
			...
			.on("mouseout", function () {
				d3.select(this)
					.attr("stroke", "lightblue") // Revert color on mouse out
					.attr("opacity", 0.6); // Revert opacity
